...
{
    // Print number of pages available for debugging
    printf("Allocating %d pages\n", npages);
    printf("Total pages available: %d\n", _num_pages);

    // Check if requested pages exceed the available pages
    ...
    // Iterate through all pages to find contiguous free pages
    for (int i = 0; i <= (_num_pages - npages); i++) {
        if (_is_free(page_i)) {
            int found = 1;

            // Check if the next npages - 1 pages are also free
            ...
            // If contiguous free pages are found
            if (found) {
                ...
                // Mark the last page of the block
                page_k--;
                _set_flag(page_k, PAGE_LAST);

                // Return the actual address of the first page
    ...
    // If no contiguous free pages were found, return NULL
    return NULL;
}
